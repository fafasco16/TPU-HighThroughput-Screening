...
因此，来源可靠的 DFT、AIMD、经典 MD、粗粒化 MD、有限元结果、规则枚举结构以及模型预测都可以进入相应 Gold 子层。协议不完整的数值仍原样保留在条件参考层；只有准备把它用于某一性质的直接监督时，才要求通过该任务的单位、映射、适用域、收敛和泄漏检查。
...
总账中的 `weight_ceiling` 统一解释为“缺口闭合后的潜在权重上限”，不是当前训练权重。`conditional_reference` 可以保留非零潜在上限，方便记录单位、协议或映射补齐后的计划，但其当前有效监督权重恒为0；只有升级为 `admitted_reference`，并通过具体任务、训练权利、防泄漏划分，达到 `model_ready=true` 且完成权重物化后，才可进入损失函数。
...
当前已经物化两个可直接查询、但仍属于参考阶段的 Gold 视图：

1. `结果/Gold_候选.csv`：13,544 条结构级 `Gold-V` 参考，含规范 SMILES、InChIKey、官能团角色、筛选范围、来源和零监督权重；
2. `结果/Gold_C_计算性能.csv.gz`：210,345 条计算数值，含结构族键、性质、数值、单位状态、方法家族、保真度、来源定位和准入状态。

它们都不是最终训练集。结构参考还没有全部形成完整 TPU 配方，计算性能也还没有全部映射到实验体系；当前训练权重为空、模型就绪数为 0。

在结构—配方—工艺—性能闭合后，最终只向建模和论文分析暴露四个简洁视图：

1. `Gold_配方.parquet`：组分结构、计量、合成/加工和材料状态；
2. `Gold_性能.parquet`：实验、计算或预测性能及其测试/计算条件；
3. `Gold_曲线.parquet`：曲线元数据和点长表，不把点数当独立样本数；
4. `Gold_候选.parquet`：候选结构、复筛结果、不确定度和实验验证状态。

底层数据库可以保留更细的规范表和审计证据，但日常筛选只面向这四个视图。
