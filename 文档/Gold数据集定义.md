...
2. `结果/Gold_C_计算性能.csv.gz`：1,435,243 条计算输出或明确标注的计算上下文，含结构/过程体系族键、显式 `split_group`、性质、数值、单位状态、方法家族、记录角色、保真度、来源定位和准入状态；同结构或同过程体系的属性、工况与时间点预先绑定到同一泄漏阻断组，但尚未创建训练折；
