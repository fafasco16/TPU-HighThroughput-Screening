...
| 退出码 0 与日志末行 `normal termination of xtb` | 已在本项目xTB 6.7.1 Linux二进制烟雾测试确认 | 成功门的一部分，仍需JSON/WBO与字段闭合 |
...
- xTB 非零退出、日志末行不是`normal termination of xtb`、或无有效`xtbout.json`；
...
本仓库已实现对应模块：`代码/xTB系综任务.py`、`代码/运行xTB构象任务.py`、`代码/xTB输出解析.py`和`代码/反应位点描述符.py`。实现通过单元测试不等于服务器二进制输出已经通过烟雾门；以下生产测试仍必须执行。

2026-08-24已在独立Ubuntu实例用官方xTB 6.7.1 Linux二进制完成二醇与二异氰酸酯烟雾测试。实际JSON版本为`6.7.1 (edcfbbe)`；JSON、WBO、charges、分子/逐原子`α(0)`与NCO/OH位点描述符均通过。真实单点不生成`.xtbok`，项目已据此修正成功门。完整环境与数值见[独立实例环境](../计算/独立实例环境.md)。
