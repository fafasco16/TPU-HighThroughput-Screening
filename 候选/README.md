...
当前82个构件中，46个虚拟二异氰酸酯没有PubChem供应商目录命中；14个宏二醇项虽然是可购小分子，但仍只是结构代理。因此原82构件没有完整组合通过实验硬门。商业证据表已扩到22条，19条进入现实规划构件池，形成245个基础体系和980条配方；3条PCL/PCDL产品因代表链模型未建立保持阻断。新增商业构件单独标记为`added_commercial_control`，不冒充原82构件的CREST/xTB结果。
