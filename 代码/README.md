...
拟合氨基甲酸酯扭转修正.py 对称化刚性残差并交叉验证家族分型傅里叶修正
汇总氨基甲酸酯受约束松弛.py 核验8个冻结二面角点并连接对应刚性DFT曲线
运行氨基甲酸酯MM约束松弛.py 用LAMMPS二面角约束最小化并读取解除约束后的GAFF2能量
汇总氨基甲酸酯MM约束松弛.py 核验8点MM松弛、原始哈希并连接刚性GAFF2曲线
比较氨基甲酸酯DFT_MM松弛.py 连接同角度松弛结果并保持DFT失败点阻断
构建RESP核心约束电荷诊断.py 固定RESP核心并最小L2中性化未映射原子以量化完整链电荷缺口
生成现实实验短名单.py 从12条高层候选形成校准—探索—专用分层的6条实验设计
生成实验回填模板.py 为6条短名单生成真实批次、试样、原始曲线和Gold-E回填空白模板
生成短名单新颖性初筛.py 登记开放论文/专利先例并阻断未经闭库复核的新颖性声明
```
