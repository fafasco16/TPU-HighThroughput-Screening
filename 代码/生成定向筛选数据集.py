...
- 受治理扩充包：33个、1,901行端点/记录
- 多目标材料索引：73个材料键，其中38个覆盖三类目标证据；TPUU-C/D/R/S由同来源拉伸、循环和TGA直接闭合，Cheetah与Filaflex 60A循环维度是应力松弛代理而非直接循环；EOS TPU 1301的17条拉伸与3条无身份冲突松弛曲线只计1个材料键；P1T/X1T动态网络玻璃体形成三目标迁移闭合但TPU核心权重为0；PCU85的38根湿干单纤维只计1个材料代码和循环迁移目标；12个为交联PU迁移层，6个为商业TPU应用实验层，3个为Elastollan/PCL直接形状记忆文献配方，4个为Tecoflex药物复合实验配方，2个为IIR-OH相邻PU耐久配方，1个为TPU95A历史镜像重新物化，1个为PCF20硬质PU泡沫断裂迁移材料
