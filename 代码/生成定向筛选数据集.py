...
- 受治理扩充包：67个、16,977行端点/记录
- 多目标材料索引：124个来源—材料证据行、123个唯一材料键，其中38行覆盖三类目标证据、69行覆盖至少两类；两个独立来源均映射到同一EOS TPU 1301键。SLS来源新增31个工艺条件、140个金标准试样，但不增加化学材料数；210个银层试样保持隔离。其余迁移层继续按材料/配方/状态而非曲线点计数。
...
- `SLS_TPU1301工艺拉伸端点.csv`：EOS TPU 1301的31个论文可映射SLS工艺条件、140个物理试样、1,144,295个曲线点与700个来源直接标量；19个旧XLS通过跨平台xlrd只读解析，全部条件按同一牌号同折。
- `无溶剂PU反应动力学端点.csv`：PCL/PDEA/PEA/PEG与TDI/HDI组合的21个摩尔比—温度条件，提取NCO半衰/90%消耗时间和删失状态；只服务合成可行性门，不进入机械三目标。
- `微孔PU_DMA端点.csv`：400/600/800M三密度级的tanδ峰温、指定温度E′/E″；只作热机械转变/阻尼辅助，不冒充TGA分解稳定性。
- `微孔PU_SHPB端点.csv`：三密度×30/48/62速度的9个物理冲击条件，双传感器峰值应力/冲量嵌套保存；无完整应变曲线，不冒充韧性。
- `SLS_TPU1301银层工艺拉伸端点.csv`：44个显式白名单序列、210个名义试样、643,157点；4条跨金层重复与TPU_2 (45°)整组5条异常试样权重0，相对金层新增206条唯一曲线。
- `MDPI_PU分子动力学描述符.csv`：三种polyol:MDI配比×四温度及共享多元醇参考的79条Gold-C候选描述符；结构/种子/轨迹未闭合前训练权重0。
- `PU泡沫动态压缩端点.csv`：HDB/HA两种来源泡沫代码在−20/−10/0/10/室温/40 ℃的12条动态压缩端点；HDB曲线未达到65%应变时不外推，只保留源表归一化能量统计与完整曲线积分。
- `4TU自修复TPU恢复配对.csv`：SH-TPU与Ninjaflex共10个同试样刀切—30 ℃/24 h无压修复—复测配对，保存公共位移终点下的峰值载荷与切割功恢复；不冒充拉伸韧性或循环残余应变。
- `4TU自修复TPUTGA端点.csv`：CroHeal 2000/EHD/MDI=1:0.6:1.7的SH-TPU原始聚合物与FDM丝材两条氮气TGA曲线，按同一配方分组。
- `导电自修复PU拉伸与回收端点.csv`：五种PEDOT:PSS/交联PU溶液配比的直接拉伸曲线及优化配方原始/化学回收/机械再用三状态曲线；溶液质量分数不冒充干基聚合物分数。
- `导电自修复PU恢复文献指标.csv`：两条明确的室温cut-stick断裂伸长恢复率和一条500圈能量恢复文献摘要；本地ZIP没有机械循环原始曲线，故不制造逐圈数据。
- `TPU鞋材TGA端点.csv`：eSUN eTPU、Rosh TPU 95A和Xinbo Chuan PEBA三种商业弹性体在氮气20 ℃/min下的T5/T10/T50与DTG峰；PEBA保持外部弹性体辅助身份。
- `TPU鞋材耐磨摘要.csv`：三材料×30/40/50/70%相对密度的12条200圈磨耗质量损失文献摘要；几何条件不扩张化学材料数，未公开的压缩数值曲线不物化。
- `TPU实验100pct拉伸端点.csv`：未知牌号TPU三个独立试样的0–100%应变曲线端点；曲线未断裂，只作100%应力和部分能量标签，不冒充断裂韧性。
- `TPU仿真应力应变参考.csv`：同一比较工作簿中的13条校准/参数试验曲线，按实验三重复中位曲线计算覆盖率与RMSE；求解器、网格和材料参数未闭合，训练权重固定为0。
- `木质素TPU前驱纤维力学.csv`：两种木质素与Pearlthane ECO 12T95的8个熔纺前驱体配方强度、模量和伸长摘要；两条“>200%”按右删失保存，不写成200%的实测值。
- `木质素TPU_TGA端点.csv`：TcA/TcC两类木质素×50/40/30 wt% Pearlthane的6条TGA端点；温度单位由论文闭合、工作簿时间单位保持未知。
- `TPU压缩打印DOE端点.csv`：NinjaFlex/PolyFlex共184个物理试样、344条方向化离散压缩响应，保存5/10/15/20%应力与20%离散能量代理；不宣称连续压缩曲线或断裂韧性。
- `再生PU泡沫压缩曲线.csv`与`再生PU泡沫压缩端点.csv`：再生/大豆PU泡沫59条真实压缩试样曲线、13,159个曲线点和118个10%/25%压缩端点；作为泡沫能量吸收迁移，不计入TPU核心。
- `再生PU泡沫黏度曲线.csv`、`再生PU泡沫热导端点.csv`、`再生PU泡沫配方组件.csv`和`再生PU泡沫聚合标量.csv`：581个黏度点、7条热导率、39条配方组分和27条聚合标量；分别用于工艺、热传导、配方上下文和压缩密度参考，不把黏度当反应动力学或热导率当TGA。
- `TPMS_FEA输入清单.csv`：Mendeley TPU-95A 来源的9个 Abaqus 输入卡，覆盖P/D/IWP三种拓扑和0.001/0.01/0.1 s⁻¹三种速率；当前没有仿真输出，不能当作性能标签或9个新材料。
- `植物基PU泡沫温湿老化压缩端点.csv`：60/75/90 ℃、90% RH 三组环境中的90条泡沫压缩试样端点（360,176个原始点，DIR1/DIR3各45条）；2条完全重复曲线隔离，输出仅作环境老化迁移，不生成致密 TPU 应力或断裂韧性。
- `SHPU自愈拉伸端点.csv`、`SHPU恢复加载端点.csv`和`SHPU界面韧性摘要.csv`：3个PE10/GY配方、SHPU原始/6/12/24 h自愈状态、1 h后二次加载和3条界面韧性摘要；这是超分子PU迁移层，不计入TPU核心，界面值不等于块体断裂韧性。
- `代理训练任务统计.csv`、`代理训练数据源清单.csv`和`代理模型训练输入发布清单.json`：将三目标长表、计算证据和67个扩充包整理为训练入口；代理训练可开始，但严格核心结构—性能模型仍按化学闭合数设门。
...
1. 既有开放来源和本轮高价值扩充包均已完成结构化发布；当前先启动代理模型基线和分组验证，不再为增加行数而盲目扩库。
2. 定向补齐韧性、循环恢复和热分解端点的组分—配方—工艺映射；循环滞后代理与直接恢复率始终分层。
3. 为24种商用构件录入同地区同日期报价和结构化SDS/GHS字段。
4. 增加商业二胺/胺类扩链剂后，单独生成TPUU现实候选。
5. 代理模型可以直接按`代理模型训练说明.md`启动；新量化计算、Pareto预测和最终核心模型仍需在代理预筛与购料/EHS门之后执行。
